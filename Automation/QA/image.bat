...
set CLUSTER_NAME=myefqa
set IMAGE_NAME=myefpassword
set IAM_ROLE=ecsTaskExecutionRole
...
copy iam-role.ps1 %ARTIFACTS_PATH%\%FOLDER_NAME%\%BUILD_NUMBER%\iam-role.ps1
copy install.ps1 %ARTIFACTS_PATH%\%FOLDER_NAME%\%BUILD_NUMBER%\install.ps1
...
cd %ARTIFACTS_PATH%\%FOLDER_NAME%\%BUILD_NUMBER%\Config

::if exist AppSettings.config ren AppSettings.config AppSettings-DEV.config
::if exist AppSettings-QA.config ren AppSettings-QA.config AppSettings.config

::if exist Client.config ren Client.config Client-DEV.config
::if exist Client-QA.config ren Client-QA.config Client.config

::if exist ConnectionStrings.config ren ConnectionStrings.config ConnectionStrings-DEV.config
::if exist ConnectionStrings-QA.config ren ConnectionStrings-QA.config ConnectionStrings.config
...
IF EXIST get-revision.txt DEL /F get-revision.txt
